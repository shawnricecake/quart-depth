...
from mono.model.backbones.ViT_DINO_reg import Mlp as EncoderMlp
from mono.model.backbones.ViT_DINO_reg import SwiGLUFFN as EncoderSwiGLUFFN
...
from mono.quant_fisher_gradient.quantized_module import QuantizedLayer, QuantizedBlock, PreQuantizedLayer, \
    QuantizedMatMul, QuantEncoderAttention, QuantMLPBlock, QuantSwiGLUFFNlock
...
def main_worker(cfg: dict, cali_data: list, test_data_1: list, test_data_2: list, args):
    ...
    save_path = args.save_path + "/" + \
                "fisher-{}-w_{}_{}_{}bit-a_{}_{}_{}bit.pth".format(
                    vit_name,
    ...
    recon_model(model, fp_model, cali_data, q_config.recon, cfg=cfg)
    ...
    # Save whole quantized model for convenience
    model = replace_lora_layers(model)
...
def replace_lora_layers(model):
    ...
    for name, module in model.named_children():
        # 如果是 LoRALinear，替换为 StandardLinear，并保留原始权重和偏置
        if isinstance(module, LoRALinear):
            # 创建对应的 StandardLinear 层，继承原始层的参数
            new_module = nn.Linear(module.in_features, module.out_features, bias=module.bias is not None)
            new_module.weight = module.weight  # 继承权重
            if module.bias is not None:
                new_module.bias = module.bias  # 继承偏置

            # 使用 setattr 替换原始层
            setattr(model, name, new_module)

        # 如果是 ConvLoRA，替换为 StandardConv2d，并保留原始权重和偏置
        elif isinstance(module, Conv2dLoRA):
            new_module = nn.Conv2d(module.in_channels, module.out_channels, module.kernel_size,
        ...
        # 如果是 ConvTransposeLoRA，替换为 nn.ConvTranspose2d，并保留原始权重和偏置
        elif isinstance(module, ConvTranspose2dLoRA):
            new_module = nn.ConvTranspose2d(module.in_channels, module.out_channels, module.kernel_size,
        ...
        # 如果还有子模块，递归替换
            ...
            replace_lora_layers(module)

    return model


def quantize_model(model, config_quant):
    def replace_module(module, w_qconfig, a_qconfig, special_config, q_output_or_input=True, previous_name=""):
        ...
        for name, child_module in module.named_children():
            ...
            if 'embed' in name:
                ...
                if a_qconfig.ch_axis != -1:
                    a_qconfig.ch_axis = 2
                setattr(module, name, QuantEncoderAttention(child_module, w_qconfig, a_qconfig, special_config))
            elif isinstance(child_module, (EncoderMlp)):
                if a_qconfig.ch_axis != -1:
                    a_qconfig.ch_axis = 2
                setattr(module, name, QuantMLPBlock(child_module, w_qconfig, a_qconfig))
            elif isinstance(child_module, (EncoderSwiGLUFFN)):
                if a_qconfig.ch_axis != -1:
                    a_qconfig.ch_axis = 2
                setattr(module, name, QuantSwiGLUFFNlock(child_module, w_qconfig, a_qconfig))
            elif isinstance(child_module, (nn.Linear, LoRALinear)):
                if a_qconfig.ch_axis != -1:
                    a_qconfig.ch_axis = 2
                if 'readoper.project_learn' in current_name:    # for vit-large w4a4 quantization
                    setattr(module, name, PreQuantizedLayer(child_module, None, w_qconfig, a_qconfig, q_output_or_input, special_config))
                    continue
                ...
                replace_module(child_module, w_qconfig, a_qconfig, special_config, q_output_or_input, current_name)
...
def recon_model(model, fp_model, cali_data, recon_config, cfg=None):
    if len(cali_data) > 8:
