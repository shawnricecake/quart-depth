...
from ...quant_fq.layers import QAct, QConv2d, QIntLayerNorm, QIntSoftmax, QLinear
...
class Mlp(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.drop(x)
        x = self.fc2(x)
        x = self.qact2(x)       # todo: may remove (comment)
        x = self.drop(x)
        return x
...
class Attention(nn.Module):
    def __init__(
        ...
        self.qkv = QLinear(dim, dim * 3, bias=qkv_bias,
                           ...
                           observer_str=cfg.OBSERVER_W,
                           quantizer_str=cfg.QUANTIZER_W)
        self.qact1_q = QAct(quant=quant,
                            calibrate=calibrate,
                            bit_type=cfg.BIT_TYPE_A,
                            calibration_mode=cfg.CALIBRATION_MODE_A,
                            observer_str=cfg.OBSERVER_A,
                            quantizer_str=cfg.QUANTIZER_A)
        self.qact1_k = QAct(quant=quant,
                            calibrate=calibrate,
                            bit_type=cfg.BIT_TYPE_A,
                            calibration_mode=cfg.CALIBRATION_MODE_A,
                            observer_str=cfg.OBSERVER_A,
                            quantizer_str=cfg.QUANTIZER_A)
        self.qact1_v = QAct(quant=quant,
                            calibrate=calibrate,
                            bit_type=cfg.BIT_TYPE_A,
                            calibration_mode=cfg.CALIBRATION_MODE_A,
                            observer_str=cfg.OBSERVER_A,
        ...
        self.proj = QLinear(dim, dim, bias=proj_bias,
                            quant=quant,
                            calibrate=calibrate,
                            bit_type=cfg.BIT_TYPE_W,
                            calibration_mode=cfg.CALIBRATION_MODE_W,
    ...
    def forward(self, x: Tensor, attn_bias=None) -> Tensor:
        ...
        x = self.qact2(x)
        x = self.proj(x)
        x = self.qact3(x)       # todo: may remove (comment)
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, attn_bias=None, last_quantizer=None) -> Tensor:
        def attn_residual_func(x: Tensor, attn_bias, last_quantizer) -> Tensor:
            return self.ls1(
                self.attn(
                    self.qact1(
                        self.norm1(
                            x, last_quantizer, self.qact1.quantizer
                        ) if type(self.norm1) == QIntLayerNorm else self.norm1(x)
                    ),
                    attn_bias
                )
            )
        ...
        def ffn_residual_func(x: Tensor) -> Tensor:
            return self.ls2(
                self.mlp(
                    self.qact3(
                        self.norm2(
                            x, self.qact2.quantizer, self.qact3.quantizer
                        ) if type(self.norm2) == QIntLayerNorm else self.norm2(x)
                    )
                )
            )

        x = self.qact2(x + attn_residual_func(x, attn_bias, last_quantizer))
        x = self.qact4(x + ffn_residual_func(x))
        return x
...
class BlockChunk(nn.ModuleList):
    def forward(self, x, others=None, first_quantizer=None):
        for i, b in enumerate(self):
            if others == None:
                x = b(x, last_quantizer=first_quantizer if i == 0 else self[i-1].qact4.quantizer)
            else:
                x = b(x, others, last_quantizer=first_quantizer if i == 0 else self[i-1].qact4.quantizer)
        return x
...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        quant_cfg=None,
        input_quant=False,
        int_layernorm=True,     # we can control it with QIntLayerNorm.mode 'ln' or 'int'
    ...
    def forward_features(self, x, masks=None):
        ...
        if not self.chunked_blocks:
            for i, blk in enumerate(self.blocks):
                ...
                x = blk(x=x, last_quantizer=last_quantizer)
            x_norm = self.norm(
                x, self.blocks[-1].qact4.quantizer, self.qact2.quantizer
            ) if type(self.norm) == QIntLayerNorm else self.norm(x)
        else:
            for i, blk in enumerate(self.blocks):
                first_quantizer = self.qact1.quantizer
                x = blk(x=x, first_quantizer=first_quantizer)
            x_norm = self.norm(
                x, self.blocks[0][-1].qact4.quantizer, self.qact2.quantizer
            ) if type(self.norm) == QIntLayerNorm else self.norm(x)

        x_norm = self.qact2(x_norm)
