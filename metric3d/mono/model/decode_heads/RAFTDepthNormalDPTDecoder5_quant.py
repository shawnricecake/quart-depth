...
from ...quant_fq.layers import QAct, QConv2d, QIntLayerNorm, QIntSoftmax, QLinear, QConvTranspose2d
...
class FlowHead(nn.Module):
    def __init__(self, input_dim=128, hidden_dim=256, output_dim_depth=2, output_dim_norm=4, tuning_mode=None,
        ...
        self.conv1d = QConv2d(input_dim, hidden_dim // 2, 3, padding=1,
                              quant=quant,
                              calibrate=calibrate,
        ...
        self.qact2 = QAct(quant=quant,
                          ...
                          observer_str=cfg.OBSERVER_A,
                          quantizer_str=cfg.QUANTIZER_A)
        self.conv2d = QConv2d(hidden_dim // 2, output_dim_depth, 3, padding=1,
                              quant=quant,
                              calibrate=calibrate,
                              bit_type=cfg.BIT_TYPE_W,
                              calibration_mode=cfg.CALIBRATION_MODE_W,
        ...
        self.qact3 = QAct(quant=quant,
                          ...
                          observer_str=cfg.OBSERVER_A,
                          quantizer_str=cfg.QUANTIZER_A)
        self.conv1n = QConv2d(input_dim, hidden_dim // 2, 3, padding=1,
                              quant=quant,
                              calibrate=calibrate,
                              bit_type=cfg.BIT_TYPE_W,
                              calibration_mode=cfg.CALIBRATION_MODE_W,
        ...
        self.qact4 = QAct(quant=quant,
                          ...
                          observer_str=cfg.OBSERVER_A,
                          quantizer_str=cfg.QUANTIZER_A)
        self.conv2n = QConv2d(hidden_dim // 2, output_dim_norm, 3, padding=1,
                              quant=quant,
                              calibrate=calibrate,
                              bit_type=cfg.BIT_TYPE_W,
                              calibration_mode=cfg.CALIBRATION_MODE_W,
    ...
    def forward(self, x):
        depth = self.conv2d(self.qact2(self.relu(self.conv1d(self.qact1(x)))))
        normal = self.conv2n(self.qact4(self.relu(self.conv1n(self.qact3(x)))))
...
class ConvGRU(nn.Module):
    def __init__(self, hidden_dim, input_dim, kernel_size=3, tuning_mode=None,
        ...
        self.convz = QConv2d(hidden_dim+input_dim, hidden_dim, kernel_size, padding=kernel_size//2,
                             quant=quant,
                             calibrate=calibrate,
                             bit_type=cfg.BIT_TYPE_W,
                             calibration_mode=cfg.CALIBRATION_MODE_W,
        ...
        self.qact1 = QAct(quant=quant,
                          ...
                          observer_str=cfg.OBSERVER_A,
                          quantizer_str=cfg.QUANTIZER_A)
        self.convr = QConv2d(hidden_dim+input_dim, hidden_dim, kernel_size, padding=kernel_size//2,
                             quant=quant,
                             calibrate=calibrate,
                             bit_type=cfg.BIT_TYPE_W,
                             calibration_mode=cfg.CALIBRATION_MODE_W,
        ...
        self.qact2 = QAct(quant=quant,
                          ...
                          observer_str=cfg.OBSERVER_A,
                          quantizer_str=cfg.QUANTIZER_A)
        self.convq = QConv2d(hidden_dim+input_dim, hidden_dim, kernel_size, padding=kernel_size//2,
                             quant=quant,
                             calibrate=calibrate,
                             bit_type=cfg.BIT_TYPE_W,
                             calibration_mode=cfg.CALIBRATION_MODE_W,
    ...
    def forward(self, h, cz, cr, cq, *x_list):
        ...
        z = torch.sigmoid((self.convz(self.qact1(hx)) + cz))
        r = torch.sigmoid((self.convr(self.qact2(hx)) + cr))
        q = torch.tanh((self.convq(self.qact3(torch.cat([r*h, x], dim=1))) + cq))
    ...
    def forward(self, net, inp, corr=None, flow=None, iter08=True, iter16=True, iter32=True, update=True):
        ...
        if iter08:
            if corr is not None:
                ...
                motion_features = flow      # all go into here
...
class ResidualBlock(nn.Module):
    def __init__(self, in_planes, planes, norm_fn='group', stride=1, tuning_mode=None,
        ...
        self.conv1 = QConv2d(in_planes, planes, kernel_size=3, padding=1, stride=stride,
                             quant=quant,
                             calibrate=calibrate,
                             bit_type=cfg.BIT_TYPE_W,
                             calibration_mode=cfg.CALIBRATION_MODE_W,
                             observer_str=cfg.OBSERVER_W,
                             quantizer_str=cfg.QUANTIZER_W)
        self.conv2 = QConv2d(planes, planes, kernel_size=3, padding=1,
                             quant=quant,
                             calibrate=calibrate,
                             bit_type=cfg.BIT_TYPE_W,
                             calibration_mode=cfg.CALIBRATION_MODE_W,
        ...
        elif norm_fn == 'layer':        # all use this
        ...
        if stride == 1 and in_planes == planes:
            ...
            self.downsample = nn.Sequential(
                QConv2d(in_planes, planes, kernel_size=1, stride=stride,
                        quant=quant,
                        calibrate=calibrate,
                        bit_type=cfg.BIT_TYPE_W,
                        calibration_mode=cfg.CALIBRATION_MODE_W,
    ...
    def forward(self, x):
        ...
        y = self.qact2(y)       # todo: may remove it (comment)
...
class ContextFeatureEncoder(nn.Module):
        ...
        for dim in output_dim:
            conv_out = nn.Sequential(
                ResidualBlock(in_dim[0], dim[0], 'layer', stride=1, tuning_mode=tuning_mode,
                              # quant:
                              quant=quant, calibrate=calibrate, cfg=cfg,),
                QConv2d(dim[0], dim[0], 3, padding=1,
                        quant=quant,
                        calibrate=calibrate,
                        bit_type=cfg.BIT_TYPE_W,
                        calibration_mode=cfg.CALIBRATION_MODE_W,
        ...
        for dim in output_dim:
            conv_out = nn.Sequential(
                ResidualBlock(in_dim[1], dim[1], 'layer', stride=1, tuning_mode=tuning_mode,
                              # quant:
                              quant=quant, calibrate=calibrate, cfg=cfg,),
                QConv2d(dim[1], dim[1], 3, padding=1,
                        quant=quant,
                        calibrate=calibrate,
                        bit_type=cfg.BIT_TYPE_W,
                        calibration_mode=cfg.CALIBRATION_MODE_W,
        ...
        for dim in output_dim:
            conv_out = nn.Sequential(
                ResidualBlock(in_dim[2], dim[2], 'layer', stride=1, tuning_mode=tuning_mode,
                              # quant:
                              quant=quant, calibrate=calibrate, cfg=cfg,),
                QConv2d(dim[2], dim[2], 3, padding=1,
                        quant=quant,
                        calibrate=calibrate,
                        bit_type=cfg.BIT_TYPE_W,
                        calibration_mode=cfg.CALIBRATION_MODE_W,
    ...
    def forward(self, encoder_features):
        x_4, x_8, x_16, x_32 = encoder_features     # inputs are all quantized
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, channels, tuning_mode=None,
        ...
        self.qact1 = QAct(quant=quant,
                               ...
                               observer_str=cfg.OBSERVER_A,
                               quantizer_str=cfg.QUANTIZER_A)
        self.conv1 = QConv2d(channels, channels, kernel_size=3, stride=1, padding=1,
                             quant=quant,
                             calibrate=calibrate,
                             bit_type=cfg.BIT_TYPE_W,
                             calibration_mode=cfg.CALIBRATION_MODE_W,
        ...
        self.qact2 = QAct(quant=quant,
                          ...
                          observer_str=cfg.OBSERVER_A,
                          quantizer_str=cfg.QUANTIZER_A)
        self.conv2 = QConv2d(channels, channels, kernel_size=3, stride=1, padding=1,
                             quant=quant,
                             calibrate=calibrate,
                             bit_type=cfg.BIT_TYPE_W,
                             calibration_mode=cfg.CALIBRATION_MODE_W,
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        out = x + out
        out = self.qact3(out)       # todo: may remove it (comment)
        return out
...
class FuseBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, fuse=True, upsample=True, scale_factor=2, tuning_mode=None,
        ...
        self.out_conv = QConv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0,
                                quant=quant,
                                calibrate=calibrate,
                                bit_type=cfg.BIT_TYPE_W,
                                calibration_mode=cfg.CALIBRATION_MODE_W,
    ...
    def forward(self, x1, x2=None):     # x2 is quantized input
        if x2 is not None:
            x2 = self.way_branch(x2)
            x1 = x1 + x2

        x1 = self.qact1(x1)     # todo: may remove it (comment)
        ...
        out = self.qact2(out)   # todo: may put it in `if self.upsample: `
...
class Readout(nn.Module):  
    ...
    def __init__(self, in_features, use_cls_token=True, num_register_tokens=0, tuning_mode=None,
        ...
        if self.use_cls_token:
            self.project_patch = QLinear(in_features, in_features, bias=True,
                                         quant=quant,
                                         calibrate=calibrate,
                                         bit_type=cfg.BIT_TYPE_W,
                                         calibration_mode=cfg.CALIBRATION_MODE_W,
                                         observer_str=cfg.OBSERVER_W,
                                         quantizer_str=cfg.QUANTIZER_W)
            self.project_learn = QLinear((1 + num_register_tokens) * in_features, in_features, bias=False,
                                         quant=quant,
                                         calibrate=calibrate,
                                         bit_type=cfg.BIT_TYPE_W,
                                         calibration_mode=cfg.CALIBRATION_MODE_W,
    ...
    def forward(self, x):   # here the x is quantized vit features
...
class Token2Feature(nn.Module):
    ...
    def __init__(self, vit_channel, feature_channel, scale_factor, use_cls_token=True, num_register_tokens=0, tuning_mode=None,
        ...
        if scale_factor > 1 and isinstance(scale_factor, int):
            self.sample = QConvTranspose2d(in_channels=vit_channel, out_channels=feature_channel, kernel_size=scale_factor, stride=scale_factor, padding=0,
                                           quant=quant,
                                           calibrate=calibrate,
                                           bit_type=cfg.BIT_TYPE_W,
                                           calibration_mode=cfg.CALIBRATION_MODE_W,
            ...
            self.sample = QConv2d(in_channels=vit_channel, out_channels=feature_channel, kernel_size=1, stride=1,padding=0,
                                  quant=quant,
                                  calibrate=calibrate,
                                  bit_type=cfg.BIT_TYPE_W,
                                  calibration_mode=cfg.CALIBRATION_MODE_W,
    ...
    def forward(self, x):
        ...
        x = self.sample(x)
        x = self.qact1(x)   # todo: may remove (comment)
        return x
...
class DecoderFeature(nn.Module):
    ...
    def forward(self, ref_feature):
        x, x2, x1, x0 = ref_feature # 1/14 1/14 1/7 1/4     # inputs are all quantized
...
class RAFTDepthNormalDPT5(nn.Module):
    def __init__(self, cfg,
        ...
        self.token2feature = EncoderFeature(self.in_channels[0], self.feature_channels, self.use_cls_token, self.num_register_tokens,
                                            # quant:    # todo
                                            quant=quant, calibrate=calibrate, cfg=quant_cfg,)
        ...
        self.depth_regressor = nn.Sequential(
            QConv2d(self.used_res_channel, self.num_depth_regressor_anchor, kernel_size=3, padding=1,
                    quant=quant,
                    calibrate=calibrate,
                    bit_type=quant_cfg.BIT_TYPE_W,
                    calibration_mode=quant_cfg.CALIBRATION_MODE_W,
                    observer_str=quant_cfg.OBSERVER_W,
                    quantizer_str=quant_cfg.QUANTIZER_W),
            nn.ReLU(inplace=True),
            QAct(quant=quant,
                 calibrate=calibrate,
                 bit_type=quant_cfg.BIT_TYPE_A,
                 calibration_mode=quant_cfg.CALIBRATION_MODE_A,
                 observer_str=quant_cfg.OBSERVER_A,
                 quantizer_str=quant_cfg.QUANTIZER_A),
            QConv2d(self.num_depth_regressor_anchor, self.num_depth_regressor_anchor, kernel_size=1,
                    quant=quant,
                    calibrate=calibrate,
                    bit_type=quant_cfg.BIT_TYPE_W,
                    calibration_mode=quant_cfg.CALIBRATION_MODE_W,
        ...
        self.normal_predictor = nn.Sequential(
            QConv2d(self.used_res_channel, 128, kernel_size=3, padding=1,
                    quant=quant,
                    calibrate=calibrate,
                    bit_type=quant_cfg.BIT_TYPE_W,
                    calibration_mode=quant_cfg.CALIBRATION_MODE_W,
        ...
        #     [Conv2dLoRA(self.context_dims[i], self.hidden_dims[i] * 3, 3, padding=3 // 2, r=0) for i in
        ...
        self.context_zqr_convs = nn.ModuleList([
            nn.Sequential(
                QAct(quant=quant,
                     ...
                     quantizer_str=quant_cfg.QUANTIZER_A),
                QConv2d(
                    in_channels=self.context_dims[i], out_channels=self.hidden_dims[i] * 3, kernel_size=3, padding=3 // 2,
                    quant=quant,
                    calibrate=calibrate,
                    bit_type=quant_cfg.BIT_TYPE_W,
                    calibration_mode=quant_cfg.CALIBRATION_MODE_W,
    ...
    def regress_depth(self, feature_map_d):
        ...
        return (self.clamp(d) - self.max_val)/ self.regress_scale, prob_feature # 'prob_feature' is never used behind
    ...
    def forward(self, vit_features, **kwargs):
        ...
        vit_features = vit_features[0]      # quantized
        ...
        depth_pred, binmap = self.regress_depth(feature_map) # regress bin for depth    # `depth_pred` is float when `binmap` is never used behind
        normal_pred = self.pred_normal(feature_map, normal_confidence_map) # mlp for normal     # `normal_pred` is float
        ...
        net_list = [torch.tanh(x[0]) for x in cnet_list] # x_4, x_8, x_16 of hidden state   # float here
        inp_list = [torch.relu(x[1]) for x in cnet_list] # x_4, x_8, x_16 context features  # float here

        # Rather than running the GRU's conv layers on the context features multiple times, we do it once at the beginning 
        # inp_list = [
        #     list(conv(i).split(split_size=conv.out_channels // 3, dim=1)) for i, conv in zip(inp_list, self.context_zqr_convs)
        # ]
        inp_list = [
            list(conv(i).split(split_size=conv[1].out_channels // 3, dim=1)) for i, conv in
        ...
        for itr in range(self.iters):
            ...
            if self.n_gru_layers == 3 and self.slow_fast_gru: # Update low-res GRU      # will go in
                net_list = self.update_block(net_list, inp_list, iter32=True, iter16=False, iter08=False, update=False)
            if self.n_gru_layers >= 2 and self.slow_fast_gru:# Update low-res GRU and mid-res GRU   # will go in
